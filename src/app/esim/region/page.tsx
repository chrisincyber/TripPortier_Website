...
export default function RegionalEsimPage() {
  ...
  const dataGroups = useGrouped ? groupByData(basePackages) : []

  const displayPackages = useGrouped ? basePackages : [...basePackages].sort((a, b) => {
    switch (sortBy) {
        ...
        {isExpanded && (
          <div className="border-t border-slate-100 bg-slate-50/50 p-4 sm:p-5 space-y-3">
            ...
            <div className="flex items-start gap-2 p-3 rounded-xl bg-emerald-50 border border-emerald-100">
              ...
              <p className="text-xs text-emerald-800"><strong>Activation:</strong> Your {pkg.days}-day plan starts when you first connect to a network.</p>
            </div>
              ...
              {loading ? (
                ...
                <div className="space-y-6">
                  {useGrouped ? (
                    ...
                    <div className="space-y-3">
                      {displayPackages.map(pkg => renderCard(pkg))}
                    </div>
                  )}
                </div>
              )}
